...
def zscore(word, correlated_word, db_cursor, file):
	...
	#file.write('Z-score dla dwoch slow: ' + word + ' ' + correlated_word + ' wynosi: ' + str(zscore) + '\n')
	return (zscore, common_par_list)
    else:
	return 'z-score nie zostal policzony poniewaz szukane slowo nie wystepuje w bazie danych'

def all_words_list(word, db_cursor):
    result = select_words(word, db_cursor)
    pmid_list = result[0][0].split(',')
    all_words_list = []

    for pmid in pmid_list:
	words_list =  select_paragraphs(pmid, db_cursor)[0][0].split(',')
	all_words_list += words_list
    all_words_list = list(set(all_words_list))
    return all_words_list
    #zscore(word, correlated_word, db_cursor, file)
...
def show_words_from_paragraphs(list_p, cut_v, db_cursor, file):
